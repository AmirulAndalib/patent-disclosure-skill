...
**主表只对 D1。** 按**技术维度**逐行比对，给每条实质区别一个编号 **F1、F2…**（全案唯一；第二、四、五章复用**同一编号与同一口径**）。有 D2 补行时加「对比文献」列，无则省略该列：

| 对比维度 | 对比文献 | 现有技术 | 本发明 | 是否区别 |
|----------|----------|----------|--------|----------|
| 节点资源刻画 | D1 | 单一 CPU 负载阈值 | CPU/内存/IO 多维画像 + 窗口平滑 | 是（F1） |
| 重排时机 | D1 | 每次派发前均重排 | 间隔阈值与分数变化阈值双条件触发 | 是（F2） |
| 重排范围 | D1 | 全队列 | 队列窗口子集 | 是（F3） |
| 窗口平滑 | D2（补行） | 公开指数滑动平均 | 与 F2 双阈值触发联用 | 组合风险（F2） |

**D2 补行不是第二套 D1**：仅当「D1 没有、本案有、且 D2 公开了相近手段、审查时可能拿来组合」时才加行。不要把 D2 的全部特征铺进主表，也不要对五篇做并列逐特征。无组合风险则不要 D2 行。
...
- 区别对「材料里的现有做法 / 本领域常规」写，Fk 仍可编号供第二、四、五章复用，并标明**未经对比文件钉死**。
